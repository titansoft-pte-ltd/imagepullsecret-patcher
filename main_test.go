...
func helperCreateValidSecret(k8s *k8sClient) error {
	_, err := k8s.clientset.CoreV1().Secrets(v1.NamespaceDefault).Create(context.Background(),dockerconfigSecret(v1.NamespaceDefault), metav1.CreateOptions{})
	return err
}

func helperCreateOpaqueSecret(k8s *k8sClient) error {
	_, err := k8s.clientset.CoreV1().Secrets(v1.NamespaceDefault).Create(context.Background(),&v1.Secret{
		ObjectMeta: metav1.ObjectMeta{
...
func helperCreateServiceAccountWithoutImagePullSecret(serviceAccountName string) step {
	return func(k8s *k8sClient) error {
		_, err := k8s.clientset.CoreV1().ServiceAccounts(v1.NamespaceDefault).Create(context.Background(),&v1.ServiceAccount{
			ObjectMeta: metav1.ObjectMeta{
...
func assertNoSecret(k8s *k8sClient) error {
	_, err := k8s.clientset.CoreV1().Secrets(v1.NamespaceDefault).Get(context.Background(),configSecretName, metav1.GetOptions{})
...
func assertSecretIsValid(k8s *k8sClient) error {
	secret, err := k8s.clientset.CoreV1().Secrets(v1.NamespaceDefault).Get(context.Background(),configSecretName, metav1.GetOptions{})
	if err != nil {
